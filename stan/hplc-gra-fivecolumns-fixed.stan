...
corr_L~lkj_corr_cholesky_point_lower_tri([0.568,0.781,0.55,0.719,0.554,0.918]',[0.0704,0.0413,0.0737,0.0462,0.0704,0.017]');
 ...
 for (c in 1 : (nColumns-1)) { 
  ...
  if (run_estimation == 1) {
    target += reduce_sum(partial_sum, ind[idx], grainsize, trobs[idx], steps[idx],
                         hplcparam[idx,], analyte[idx], column[idx], modifier[idx], R, logkwx, apHx,
