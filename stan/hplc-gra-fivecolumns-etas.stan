...
generated quantities {
  ...
  vector[nAnalytes] etalogkwN;
  vector[nAnalytes] etaS1mN;
  vector[nAnalytes] etadS1N;
  vector[nAnalytes] etadlogkT;
  vector[nGroupsA] etadlogkwA;
  vector[nGroupsB] etadlogkwB;
  vector[nGroupsA] etadS1mA;
  vector[nGroupsB] etadS1mB;
  vector[nGroupsA] etadS1A;
  vector[nGroupsB] etadS1B;
  
  array[nColumns-1] vector[nAnalytes] etaclogkwNc;
  array[nColumns-1] vector[nAnalytes] etacS1mNc;
  array[nColumns-1] vector[nAnalytes] etacdS1Nc;
  array[nColumns-1] vector[nAnalytes] etacdlogkTc;
  array[nColumns-1] vector[nGroupsA] etacdlogkwAc;
  array[nColumns-1] vector[nGroupsB] etacdlogkwBc;
  array[nColumns-1] vector[nGroupsA] etacdS1mAc;
  array[nColumns-1]vector[nGroupsB] etacdS1mBc;
  array[nColumns-1]vector[nGroupsA] etacdS1Ac;
  array[nColumns-1]vector[nGroupsB] etacdS1Bc;
  
   
  for(i in 1:nAnalytes){
    etalogkwN[i] = (paramN[i,1]-miu[i,1])/omega[1]; 
    etaS1mN[i] = (paramN[i,2]-miu[i,2])/omega[2];
    etadS1N[i]   = (dS1N[i]-miu[i,3])/omega[3];
    etadlogkT[i] = (dlogkT[i] -dlogkTHat)/omegaT;
  }
  
  for(d in 1:nGroupsA){ 
    etadlogkwA[d] = (dlogkwA[d] - dlogkwHat[1])/kappa[1];
    etadS1mA[d]  = (dS1mA[d]-dS1mHat[1])/kappa[2];
    etadS1A[d]   = (dS1A[d]-ddS1Hat[1])/kappa[3];
  }
  
  for(d in 1:nGroupsB){ 
    etadlogkwB[d] = (dlogkwB[d]-dlogkwHat[2])/kappa[1];
    etadS1mB[d] = (dS1mB[d]-dS1mHat[2])/kappa[2];
    etadS1B[d] = (dS1B[d]-ddS1Hat[2])/kappa[3];
  }
   
    for(c in 1 : (nColumns-1)) {
    for(i in 1:nAnalytes){
    etaclogkwNc[c,i] = (clogkwN[c,i]-cmiu[c,i,1])/comega[c,1]; 
    etacS1mNc[c,i] = (cS1mN[c,i]-cmiu[c,i,2])/comega[c,2];
    etacdS1Nc[c,i]   = (cdS1N[c,i]-cmiu[c,i,3])/comega[c,3];
    etacdlogkTc[c,i]   = etacdlogkT[c,i];
    }
    for(d in 1:nGroupsA){ 
    etacdlogkwAc[c,d] = etacdlogkwA[c,d];
