...
// 2nd column
  ...
  for(i in 1:Nsim){
    paramNPred[i] = miuPred[i,1:2]; 
    dS1NPred[i]   =miuPred[i,3];
     
    dlogkwAPred[i] = dlogkwHat[1];
    dS1mAPred[i] = dS1mHat[1];
    dS1APred[i] = ddS1Hat[1];
    
    dlogkwBPred[i] = dlogkwHat[2];
    dS1mBPred[i] = dS1mHat[2];
    dS1BPred[i] = ddS1Hat[2];
  }
  
  
  for(i in 1:Nsim){
    for (c in 1 : (nColumns-1)) {
    clogkwNPred[c,i] = cmiuPred[i, 1, c];
    cS1mNPred[c,i]     = cmiuPred[i,2,c];
    cdS1NPred[c,i]   = cmiuPred[i,3,c];
    cdlogkwAPred[c,i] = cdlogkwHat[c,1];
    cdS1mAPred[c,i] = cdS1mHat[c,1];
    cdS1APred[c,i] = cddS1Hat[c,1];
    cdlogkwBPred[c,i] = cdlogkwHat[c,2];
    cdS1mBPred[c,i] = cdS1mHat[c,2];
    cdS1BPred[c,i] = cddS1Hat[c,2];
  }}
