...
// 1st column
  ...
  // 2nd column
  matrix[nColumns-1, nAnalytes] etaclogkwNStdPred;
  matrix[nColumns-1, nAnalytes] etaclogkwNPred;
  ...
  for (c in 1 : (nColumns-1)) {
    for(i in 1:nAnalytes){
    clogkwNPred[c,i] = cmiu[i, 1, c]  + etaclogkwNPred[c,i];
    cS1mNPred[c,i] =normal_rng(cmiu[i,2,c], comega[c,2]);
    cdS1NPred[c,i]   = normal_rng(cmiu[i,3,c], comega[c,3]);
 ...
 for(z in 1:nObs){
  ...
  real trHatCond = chromgratrapz(steps[z], 
                                ...
                                R[analyte[z]],
                                hplcparam[z]);
  real trHatPred = chromgratrapz(steps[z], 
  ...
  trCond[z] = student_t_rng(3, trHatCond, sigmax[analyte[z],column[z]]);
  trPred[z] = student_t_rng(3, trHatPred, sigmaxPred[analyte[z],column[z]]);
  }
