...
## The expected utylity map (similarity)

```{r casestudy1-similarity, eval = FALSE}

datastructsim_red_similarity <- datastructsim_red
datastructsim_red_similarity$tr_ref <- c(2.470840,5.748797,22.643939,	19.343264,16.959259,8.316534)

model_sim_red <- cmdstan_model("stan/hplc-gra-fivecolumns-fixed-similarity.stan")

fit_sim_red2  <- model_sim_red$generate_quantities(fit_red1,
                                         data = datastructsim_red_similarity,
                                         seed = 123,
                                         parallel_chains = 4,
                                         output_dir = "stanfiles")
```

```{r casestudy1-similarity-loaddata}
x<- cmdstanr::read_cmdstan_csv(c(
                                  'stanfiles/hplc-gra-fivecolumns-fixed-similarity-202309101702-1-467fab.csv'
                                ))

draws_sim_red2_df <- as_draws_df(x$generated_quantities)
#draws_sim_red_df <- fit_sim_red$draws(format = "df")
```

The results are presented as a utility map for each of the column. The maximum is displayed below as a table

```{r casestudy1-utylitymaps-alldata-similarity}

Uresults<-datasim_red[!duplicated(datasim_red$expid),]

u<-draws_sim_red2_df[,which(colnames(draws_sim_red2_df) %in% grep("^utility", names(draws_sim_red2_df), value = TRUE))]
