---
title: "Comparison of chromatographic stationary phases using a Bayesian-based multilevel model"
author:
  - name: "Paweł Wiczling*"
    affiliations:
...
In this work $\small z$ = 1,...,`r nObs` denotes observation, $\small i$=1,...,`r nAnalytes` analyte, $\small col$=1,...,5 column, $\small m$=1,...,2 organic modifier and $\small r$=1,...,$\small R[i]$  dissociation step for $\small i$-th analyte. The observed retention times ($\small t_{Robs,z}$) were described using the following model:
...
Eta plots shows the centered and standardized individual parameters (e.g. $\eta_{logkwN,i} = (logkwN_i-(\widehat{logkwN} + \beta_1 \cdot \log P_i))/\omega_1$). They allow to visualize the unexplained between-analyte variability of chromatographic parameters.
...
The results are presented as an expected utility map for each of the column. The maximum is displayed below as a table
