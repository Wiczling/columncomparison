...
Parameters characterizing column effects:
...
For that purpose, we will use decision theory. We start by specifying a notion of utility to each outcome (uncertainty chromatogram). The Bayesian-based optimal decision is the one that maximizes expected utility. Our utility function includes the lowest retention time across analytes ($\small mintr$), the highest retention time across analytes $\small maxtr$, and the difference in retention times between the critical pair of analytes ($\small res$). This utility is zero if at least one of the analytes has retention higher than 40, less than 2 min, or the difference in retention times is less than 2. Otherwise, it favors shorter runs. (.) denotes design variables.
...
$$
To find the maximum we use a grid search over a dense set of possible chromatographic conditions:
...
The results are presented as a utility map for each of the column. The maximum is displayed below as  a table taking into consideration mobile phase pH<7.
...
cat("1. The best for XBridge Shield RP18:")
 ...
 kable(foo1[which(foo1$EUtility==max(foo1$EUtility)),c('tg','fio', 'fik','Mod', 'pHid','pHo','Temp','ColumnName','EUtility')])

cat("2. The best choice for XTerra MS C18:")
 foo2<-foo[foo$Column==2& foo$pHo<7,];
 kable(foo2[which(foo2$EUtility==max(foo2$EUtility)),c('tg','fio', 'fik','Mod', 'pHid','pHo','Temp','ColumnName','EUtility')])

cat("3. The best choice for XBridge Phenyl:")
 foo3<-foo[foo$Column==3& foo$pHo<7,];
 kable(foo3[which(foo3$EUtility==max(foo3$EUtility)),c('tg','fio', 'fik','Mod', 'pHid','pHo','Temp','ColumnName','EUtility')])

cat("4. The best choice for XBridge C8:")
 foo4<-foo[foo$Column==4& foo$pHo<7,];
 kable(foo4[which(foo4$EUtility==max(foo4$EUtility)),c('tg','fio', 'fik','Mod', 'pHid','pHo','Temp','ColumnName','EUtility')])

cat("5. The best choice for Xterra MS C8:")
 foo5<-foo[foo$Column==5& foo$pHo<7,];
 kable(foo5[which(foo5$EUtility==max(foo5$EUtility)),c('tg','fio', 'fik','Mod', 'pHid','pHo','Temp','ColumnName','EUtility')])
...
if (i==3){
 ...
 idx <- which(datasim_red$METID %in% analyte_ID_sample &
               datasim_red$tg == 60 & # c(30, 90, 270)
               datasim_red$pHid == 12 &   # c(1:9)
               datasim_red$fio == 0.15 &   # ...
               datasim_red$Column == i &  # c(1, 2)
               datasim_red$Mod2 == 1 & # c(1, 2)
               datasim_red$Temp == 25) # c(25, 35)
...
Large retention time datasets and multilevel modeling allow to characterize RP HPLC stationary phases.
