...
datasim %>%
  subset(METID==140&tg==30&Temp==25&pH==1)%>%
  select(METID, Mod, ColumnName, trCond_l, trCond_m, trCond_h, trPred_l, trPred_m, trPred_h)%>%
...
datasim_red %>%
  subset(METID==140&tg==30&Temp==25&pH==1)%>%
  select(METID, Mod, ColumnName, trCond_l, trCond_m, trCond_h)%>%
  mutate_if(is.numeric, round, 1) %>% 
  kable()
```
